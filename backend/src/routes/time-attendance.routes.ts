...
// Get current status (active time entry)
router.get('/current-status', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await query(
...
// Start Break
router.post('/start-break', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await query(
      `UPDATE time_entries
       SET on_break = true, break_start_time = NOW()
       WHERE user_id = $1 AND clock_out_time IS NULL AND on_break = false
       RETURNING *`,
      [req.user!.id]
    );
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ 
        success: false,
        message: 'No active time entry found or already on break' 
      });
      return;
    }
...
// End Break
router.post('/end-break', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await query(
      `UPDATE time_entries
       SET on_break = false,
           break_minutes = COALESCE(break_minutes, 0) + 
             EXTRACT(EPOCH FROM (NOW() - break_start_time)) / 60.0
       WHERE user_id = $1 AND clock_out_time IS NULL AND on_break = true
       RETURNING *`,
      [req.user!.id]
    );
    ...
    res.json({
      ...
      breakMinutes: result.rows[0].break_minutes
    });
  } catch (error) {
