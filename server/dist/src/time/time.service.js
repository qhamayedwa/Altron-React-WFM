...
let TimeService = class TimeService {
    ...
    async clockOut(userId, dto) {
        ...
        const totalHours = (clockOutTime.getTime() - activeEntry.clock_in_time.getTime()) / (1000 * 60 * 60);
    ...
    async getCurrentStatus(userId) {
        ...
        if (activeEntry) {
            const currentDuration = (new Date().getTime() - activeEntry.clock_in_time.getTime()) / (1000 * 60 * 60);
