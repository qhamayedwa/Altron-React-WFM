...
let OrganizationService = class OrganizationService {
    ...
    async deleteRegion(id) {
        ...
        if (region.sites.length > 0) {
            throw new common_1.BadRequestException(`Cannot delete region: ${region.sites.length} active sites still exist. Please deactivate or move sites first.`);
        }
    ...
    async findSite(id) {
        ...
        const departmentDetails = await Promise.all(site.departments.map(async (dept) => {
    ...
    async getHierarchy(companyId) {
        ...
        const hierarchy = {
            ...
            regions: await Promise.all(company.regions.map(async (region) => ({
                id: region.id,
                name: region.name,
                code: region.code,
                sites: await Promise.all(region.sites.map(async (site) => ({
                    id: site.id,
                    name: site.name,
                    code: site.code,
                    departments: await Promise.all(site.departments.map(async (dept) => {
                        const employeeCount = await this.prisma.users.count({
                            where: { department_id: dept.id, is_active: true },
                        });
                        return {
